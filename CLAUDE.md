...
| `.section-inline-cta` | Línea de CTA secundaria (texto corto + un `.btn-ghost` con `data-open-modal`) con borde superior sutil, para cerrar un bloque de contenido largo (ej. una grilla de servicios) sin esperar al CTA final de la página. No reemplaza el CTA principal — es un empujón intermedio, uno por página como máximo. |
